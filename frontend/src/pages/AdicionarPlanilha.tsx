...
export default function AdicionarPlanilha() {
  ...
  const fetchUploads = () => {
    ...
    fetch('http://localhost:3000/uploads?tipo=SPLITWISE_CSV')
      ...
      .catch(() => addNotification({ title: 'Erro', message: 'Não foi possível carregar o histórico de uploads.', color: 'red' }))
    ...
    <>
      ...
      <Paper withBorder shadow="md" p={30} mt={30} radius="md">
        ...
        <Text size="sm" c="dimmed" mt={4}>
            O ficheiro deve ser um .csv exportado do Splitwise ou com o mesmo formato.
        </Text>
        <Group mt="md">
      ...
      <Paper withBorder shadow="md" p={30} mt={30} radius="md">
         ...
         {carregando ? (
            ...
            <List spacing="xs" size="sm" center mt="lg" icon={<ThemeIcon color="green" size={24} radius="xl"><IconCsv size="1rem" /></ThemeIcon>}>
                {listaDeUploads.length > 0 ? (
                    listaDeUploads.map((upload) => (
                        <List.Item key={upload.id}>
                            {upload.nomeArquivo} - <Text span c="dimmed" size="xs">em {new Date(upload.dataUpload).toLocaleString()}</Text>
                        </List.Item>
                    ))
                ) : (
